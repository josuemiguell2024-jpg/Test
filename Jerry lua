...
-- // THEME (ENVY STYLE - BLACK + WHITE)
local BG         = Color3.fromRGB(0, 0, 0)
local CARD_BG    = Color3.fromRGB(14, 14, 14)
local CARD_HOV   = Color3.fromRGB(24, 24, 24)
...
local function saveConfig()
    ...
    pcall(function() 
        writefile("jerryAutoBatDesyncConfig.json", HttpService:JSONEncode(cfg)) 
    end)
end

local function loadConfig()
    local hasFile = false
    pcall(function() hasFile = isfile("jerryAutoBatDesyncConfig.json") end)
    if not hasFile then return end
    
    local ok, cfg = pcall(function() 
        return HttpService:JSONDecode(readfile("jerryAutoBatDesyncConfig.json")) 
    end)
...
-- // CLEANUP OLD GUI
for _, name in pairs({"jerryAutoBatDesyncGUI", "MwvaneNewaBatDesyncGUI", "PhazeAutoBatDesyncGUI"}) do
...
gui.Name="jerryAutoBatDesyncGUI"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
...
titleLbl.BackgroundTransparency=1; titleLbl.Text="JERRY ANTI DESYNC"
titleLbl.TextColor3=WHITE; titleLbl.Font=Enum.Font.GothamBlack; titleLbl.TextSize=10
...
toggleLbl.BackgroundTransparency=1; toggleLbl.Text="ANTI DESYNC"; toggleLbl.TextColor3=WHITE
...
print("[jerry Auto Bat Desync] Loaded!")
